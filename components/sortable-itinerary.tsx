...
function SortableItem({ item }: { item: Location }) {
    ...
    return (
        <div
            ...
            className="p-4 border rounded-md flex justify-between items-center hover:shadow transition-shaodw">
            <div>
...
export default function SortableItinerary({ locations, tripId, }: SortableItineraryProps) {
    ...
    const handleDragEnd = async (event: DragEndEvent) => {
        ...
        if (active.id !== over?.id) {
            ...
            const newLocationOrder = arrayMove(localLocation, oldIndex, newIndex).map((item, index) => ({ ...item, order: index }));

            setLocalLocation(newLocationOrder);

            await reorderItinerary(tripId, newLocationOrder.map((item) => item.id))
        }
    }
