...
export default async function TripsPage() {
    ...
    if (!session) {
        return (
            <div className="flex justify-center items-center h-screen text-gray-700 text-xl">Please Sign In</div>
        )
    }
    return (
                ...
                <CardContent>
                    <p>
                        {trips.length === 0
                            ...
                            } created! ${upcomingTrips.length > 0
                                ? `${upcomingTrips.length} upcoming. Way to go!`
                                : ""
                            } `}
                    </p>
                </CardContent>
                            ...
                            <Link key={key} href={`/trips/${trip.id}`}>
                                <Card className="h-full hover:shadow-md transition-shadow">
                                    ...
                                    <CardContent>
                                        ...
                                        <div className="text-sm">{new Date(trip.startDate).toLocaleDateString()} -
